...
public class APIClient {
    ...
    private static final OkHttpClient client = new OkHttpClient();
        ...
        public static Map<Long, String> getStreetsInNeighborhood(String neighborhood, String city) throws Exception {
            ...
            // Adicionar 3600000000 para obter o ID da área no contexto de pesquisa da Overpass API
            areaId += 3600000000L;
        ...
        /**
     * Faz requisições para a API OpenRouteService para criar um percurso a partir de uma lista de coordenadas.
     *
     * @param coordinates Lista de pares de coordenadas no formato [ [lon1, lat1], [lon2, lat2], ... ]
    ...
    public static JSONObject createRoute(List<double[]> coordinates) throws IOException {
        ...
        for (int i = 0; i < coordinates.size() - 1; i++) {
            ...
            if (start.length != 2 || end.length != 2) {
                throw new IllegalArgumentException("Cada par de coordenadas deve conter exatamente dois valores (longitude e latitude).");
            }
            ...
            try (Response response = client.newCall(request).execute()) {
                if (!response.isSuccessful()) {
                    throw new IOException("Erro na requisição: Código " + response.code() + " - " + response.body().string());
                }
                ...
                if (!jsonResponse.has("features")) {
                    throw new JSONException("A resposta JSON não contém a chave 'features'." + jsonResponse);
                }
    ...
    public static void main(String[] args) {
        try {
            // Lista de pares de coordenadas [lon, lat]
            ...
            // Criar o GeoJSON com o percurso
            JSONObject geoJson = createRoute(coordinates);

            // Salvar o GeoJSON em um arquivo
            saveGeoJsonToFile(geoJson, "caminho.geojson");
